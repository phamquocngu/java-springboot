...
public class JwtUtils {
  ...
  public Jws<Claims> parseToken(String token) {
    try {
      ...
      throw new JwtException(AuthConstants.Error.INVALID_JWT_SIGNATURE, e.getMessage());
    } catch (MalformedJwtException e) {
      log.error("Invalid JWT token: {}", e.getMessage());
      throw new JwtException(AuthConstants.Error.INVALID_JWT_TOKEN, e.getMessage());
    } catch (ExpiredJwtException e) {
      log.error("JWT token is expired: {}", e.getMessage());
      throw new JwtException(AuthConstants.Error.JWT_TOKEN_EXPIRED, e.getMessage());
    } catch (UnsupportedJwtException e) {
      log.error("JWT token is unsupported: {}", e.getMessage());
      throw new JwtException(AuthConstants.Error.JWT_TOKEN_UNSUPPORTED, e.getMessage());
