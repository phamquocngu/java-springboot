...
public class SwaggerPageableConfig implements OperationBuilderPlugin {
    private final TypeResolver resolver;

    @Autowired
    public SwaggerPageableConfig(TypeResolver resolver) {
        ...
        for (ResolvedMethodParameter methodParameter : methodParameters) {
            ...
            if (pageableType.equals(resolvedType)) {
                ...
                parameters.add(new RequestParameterBuilder()
                                       .in(ParameterType.QUERY)
                                       .name("sort")
                                       .query(q -> q.model(m -> m.collectionModel(c -> c.model(cm -> cm.scalarModel(
                                               ScalarType.STRING)))))
